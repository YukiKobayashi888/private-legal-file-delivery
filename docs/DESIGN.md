...
> Design notes for **private-legal-file-delivery** — a runnable typescript example that TypeScript example for expiring legal matter document links and deadline follow-up.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.storage.bucket.get(...)`).
- **Feature code** — the actual task: private-legal-document-delivery.
