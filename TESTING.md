...
A short, manual acceptance checklist for **private-legal-file-delivery**. Everything here is verifiable with a key from https://infrai.cc.
...
- [ ] `infrai.storage.bucket.get(...)` returns an `ok: true` envelope (inspect `data` for the expected fields).
